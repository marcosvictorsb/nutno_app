<template>
    <Dialog :visible="visible" header="Editar Medida" :modal="true" :style="{ width: '90vw', maxHeight: '90vh' }" :breakpoints="{ '1199px': '95vw', '575px': '100vw' }" @hide="handleClose" @update:visible="handleVisibilityUpdate">
        ...
        <template #footer>
            ...
            <Button label="Atualizar Medida" severity="success" icon="pi pi-check" :loading="loading" @click="handleSave" />
        </template>
    </Dialog>
</template>
...
watch(
    ...
    (visible) => {
        if (visible && props.medida) {
            ...
            nextTick(() => {
                // Converter data_avaliacao para Date object se for string
                if (formularioLocal.value.data_avaliacao) {
                    if (typeof formularioLocal.value.data_avaliacao === 'string') {
                        formularioLocal.value.data_avaliacao = new Date(formularioLocal.value.data_avaliacao);
                    }
                } else {
...
const handleSave = () => {
    ...
    emit('salvar-edicao-medida', payload);
};
</script>
